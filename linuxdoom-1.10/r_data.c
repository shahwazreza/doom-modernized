...
typedef struct
{
    ...
    short		width;
    short		height;
    int                 columndirectory;        // OBSOLETE (was pointer, fixed for 64-bit)
    short		patchcount;
...
void R_InitTextures (void)
{
	...
	for (j=0 ; j<texture->patchcount ; j++, mpatch++, patch++)
	{
	    ...
	    patch->originy = SHORT(mpatch->originy);
            {
                ...
                if (pidx < 0 || pidx >= nummappatches) {
                    fprintf(stderr, "Warning: bad patch index %d (max %d) in texture %s\n", pidx, nummappatches, texture->name);
                    patch->patch = 0;
                } else {
	    ...
	    if (patch->patch == -1)
	    {
                fprintf(stderr, "Warning: R_InitTextures: Missing patch in texture %s (ignored)\n",
                        texture->name);
                patch->patch = 0; // use first available patch as fallback
	    }
	}		
